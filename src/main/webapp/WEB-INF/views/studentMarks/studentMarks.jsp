					...
					<div class="centered">
						...
						<form:form action="${action}" method="${method}"
							...
							<c:if test="${showTab eq 'list'}">
								...
								<form:select path="yearId" name="yearId" id="yearId" multiple="false" cssClass="span1"
								...
								<form:select path="semesterId" name="semesterId" id="semesterId" multiple="false"
											 cssClass="span1"
									...
									<c:forEach items="${semesters}" var="semester">
										<option value="${semester.key}"
											<c:if test="${semester.key eq selectedSemesterId}"> selected="true"</c:if>
										> ${semester.value}
										</option>
									</c:forEach>
								</form:select>
								...
								<form:select path="branchId" name="branchId" id="branchId" multiple="false"
											 cssClass="span2"
											 onChange="getData('getSubjects', branchId.id, semesterId.id, subjectId.id)">
									<form:option value="0" label="Select" />
								...
								<form:select path="subjectId" name="subjectId" id="subjectId" multiple="false"
											 cssClass="span2"
											 onChange="getData('getExamTypes', this.id, 0, examTypeId.id)">
									<form:option value="0" label="Select" />
									<c:forEach items="${subjects}" var="subject">
										<option value="${subject.key}"
												<c:if test="${subject.key eq selectedSubjectId}">selected="true"</c:if>
										> ${subject.value}
										</option>
									</c:forEach>
								...
								<form:select path="examTypeId" name="examTypeId" id="examTypeId" multiple="false"
											 cssClass="span2" onchange="submitData('${Role}/student/studentMarks${operation}','studentListForm')">
									<form:option value="0" label="Select" />
									<%--<form:options items="${examTypes}" />--%>

									<c:forEach items="${examTypes}" var="examType">
										<option value="${examType.key}"
											<c:if test="${examType.key eq selectedExamTypeId}"> selected="true"</c:if>
										> ${examType.value}
										</option>
									</c:forEach>
							...
							<c:if test="${not empty studentMarksDto.marksDtos}">
								<table class="table " id="studentList" >
									<thead>
										...
										<th scope="row" style="text-align: center">Action
											<br>
									...
									<c:forEach items="${studentMarksDto.marksDtos}" var="marksDto" varStatus="row">
										<tr>
											<td>${row.index+1}
												<form:hidden
											...
											<td style="text-align: center">
												<c:choose>
													<c:when test="${marksDto.maxMarks != 0}"> ${marksDto.maxMarks }

														<form:hidden
																path="marksDtos[${row.index}].maxMarks"
																name="marksDtos[${row.index}].maxMarks"  />
													</c:when>
													<c:when test="${marksDto.maxMarks == 0}">
											...
											<td style="text-align: center">
												<c:if test="${marksDto.studentMarksId gt 0}">
													${marksDto.marks }
													<form:hidden
															path="marksDtos[${row.index}].marks"
															name="marksDtos[${row.index}].marks"  />
												</c:if>
												<c:if test="${marksDto.studentMarksId eq 0}">
													<form:input
															path="marksDtos[${row.index}].marks"
															name="marksDtos[${row.index}].marks"
											...
											<td style="text-align: center">
												<c:if test="${marksDto.studentMarksId gt 0}">
													...
													<a href="JavaScript:void(0);"
													   ...
													   onclick="edit('${Role}/student/studentMarks/edit/${marksDto.studentMarksId}/update','POST')">Edit</a>
													</a>
													<span style="padding: 10px"></span>

													<a href="JavaScript:void(0);" style="color: red"
													   onclick="edit('${Role}/student/studentMarks/edit/${marksDto.studentMarksId}/delete','POST')">Delete</a>
													</a>

												</c:if>
												<c:if test="${marksDto.studentMarksId eq 0}">
													<form:checkbox path="studentIds" value="${marksDto.studentId}" cssClass="checkbox"/>
												</c:if>
											</td>
										</tr>
									</c:forEach>
								...
								<button type="button" class="btn btn-primary"
										onclick="submitData('${Role}/student/studentMarks/add','studentListForm')">
									Post Marks
								</button>
							</c:if>
							...
							<c:if test="${showTab eq 'edit'}">
								<table class="table " id="studentList" >
									...
									<tr>
										<td>
											<form:hidden path="studentMarksId" name="studentMarksId" />
										...
										<td>${studentMarksDto.branch }
											...
											<form:hidden path="branch" name="branch" />
										</td>
										...
										<td>${studentMarksDto.semester }
											...
											<form:hidden path="semester" name="semester"  />
										</td>
										...
										<td>${studentMarksDto.rollNo }
											<form:hidden path="rollNo" name="rollNo" />
										</td>

										<td>${studentMarksDto.name }
											<form:hidden path="studentId" name="studentId"  />
											<form:hidden path="name" name="name"  />
										</td>

										<td style="text-align: center">
											<form:input path="maxMarks" name="maxMarks" id="maxMarks"
														cssStyle="text-align: right" maxlength="3" cssClass="span1"
														onkeyup="intOnly(this)"/>
										</td>
										<td style="text-align: center">
